...
def qc(adata):
    ...
    keys = 'total_counts,n_genes_by_counts,pct_counts_mt,pct_counts_ribo,pct_counts_hb'.split(
        ',')
    assert pd.Series(keys).isin(adata.obs.columns)\
        .all(), '[Error] please defind {}'.format(','.join(keys))
    ...
    with Block('qc violin', context=dict(x=2, y=y)) as context:
        ...
        for k, ylabel, ax in zip(
            ...
            sc.pl.violin(adata, k, ax=ax, ylabel=ylabel,
                         jitter=0.25, show=False)

    y += 6
    with Block('qc scatter', context=dict(x=1.5, y=y)) as context:
        a4p.area_update(context.x, context.y, 2, 3, 5,
                        5, gap_width=1.5, gap_height=1.5)

        sc.pl.scatter(adata, 'total_counts', 'n_genes_by_counts', show=False,
                      ax=a4p.area_get_ax(0, rc=pl.rc_frame))
        sc.pl.scatter(adata, 'total_counts', 'pct_counts_mt', show=False,
                      ax=a4p.area_get_ax(3, rc=pl.rc_frame))
        sc.pl.scatter(adata, 'total_counts', 'pct_counts_ribo', show=False,
                      ax=a4p.area_get_ax(4, rc=pl.rc_frame))
        sc.pl.scatter(adata, 'total_counts', 'pct_counts_hb', show=False,
...
def standard_process(adata,
    ...
    with Block('batch effect', context=dict(x=0, y=y)) as context:
        ...
        for ax, k in zip(a4p.area_yield_ax(rc=pl.rc_blank),
                         'X_pca,X_pca_harmony'.split(',')):
            if not k in adata.obsm_keys():
                continue
            umap(adata, key=key_batch, cmap=cmap,
                 ax=ax, key_obsm=k, **kw_pl_umap)
            ax.set_title(k)
    ...
    with Block('cluster', context=dict(x=0, y=y)) as context:
        ...
        for (_x, _y, ax), k in zip(list(a4p.area_yield_ax(rc=pl.rc_blank, with_xy=True)),
                                   [key_cluster, 'cell_type']
                                   ):
            if not k in adata.obs_keys():
                continue
            cmap = pl.cmap.ggsci.get_cmap('IGV', adata.obs[k].unique())
            umap(adata, key=k, cmap=cmap, ax=ax,
                 key_obsm='X_umap', **kw_pl_umap)
            ax.set_title(k)

            a4p.area_update(_x+7, _y, 1, 3, 1, 6, gap_width=2)
            for ax, k in zip(a4p.area_yield_ax(rc=pl.rc_blank),
                             np.array_split(list(cmap.keys()), 3)
                             ):
                pl.cmap.show(subset_dict(cmap, k), ax=ax)
...
def umap(adata, key, ax, ax_cmap=None, cmap=None, marker='.', size=1,
         sort=False, kw_scatter=None, key_obsm='X_umap',
         text_on_data=False, text_fontdict=None):
    ...
    assert __assert_all_value_in_cmap(
        adata, key, cmap), "[Error] not all ele of adata.obs[key] in cmap.keys()"
    ...
    df_plot = df_plot.join(
        adata.obs.loc[:, [key]].rename(columns={key: '_key'})
    )
    df_plot['_key'] = df_plot['_key'].astype(str)
    # scatter
    if sort:
        df_plot = df_plot.sort_values('_key', key=lambda s: s.map(
            {k: i for i, k in enumerate(cmap.keys())}))
    else:
    ...
    if text_on_data:
        text_fontdict = update_dict(
            dict(va='center', ha='center', fontsize=8),
            text_fontdict)
        df_text = df_plot.groupby('_key').agg(
            {'_obsm1': 'mean', '_obsm2': 'mean'})
...
def umap_gene(adata, key, ax, marker='.',
    ...
    df_plot = df_plot.sort_values(key) if sort else df_plot.sample(
        df_plot.shape[0], replace=False)
    # scatter
