...
def tl_get_significance_marker(pvalues, markers=None, not_significance_marker='ns'):
    ...
    for i, row in marker.iterrows():
        res['marker'] = res['marker'].mask(
            res['pvalue'] < row['value'], row['marker'])
...
def tl_GaussianKDE_evaluate(X, X_evaluate=None, bw_method='scott', return_X_evaluate=False):
    X_evaluate = np.arange(
        X.min(), X.max(), (X.max()-X.min())/50) if X_evaluate is None else X_evaluate
...
def tl_set_xticklabels(ax, fontdict, **kwargs):
    ...
    ax.set_xticklabels(
        [tick.get_text() for tick in ticks], fontdict=fontdict
    )
...
def scatter_confidence_bands(X, Y, ax, kvarg_ax_plot=None, kvarg_ax_fill_between=None):
    ...
    kvarg_ax_fill_between = dict(
        alpha=.5) if kvarg_ax_fill_between is None else kvarg_ax_fill_between
...
def bar_transpose_df_cumsum(df_plot, cumsum_prefix='bottom_', reindex=False):
    ...
    df_cumsum.index = df_cumsum.index.to_series().str.replace(
        '^{}'.format(cumsum_prefix), '', regex=True).to_numpy()
...
def violinplot(df_plot, ax, positions=None, widths=None, vertical=True,
    ...
    df_para['positions'] = np.arange(
        df_para.shape[0]) + 1 if positions is None else positions
    df_para['widths'] = np.repeat(.5, df_para.shape[0]
...
def bubble(df_size, ax, df_color='pink', fontdict_xtick=None, fontdict_ytick=None,
    ...
    df_plot['y'] = df_size.shape[1] - \
        np.arange(df_size.size) % df_size.shape[1]
    ...
    ax.set_yticks(np.arange(df_size.shape[1])+1,
                  df_size.columns[::-1], **fontdict_ytick)
    ax.set_xticks(np.arange(df_size.shape[0])+1,
                  df_size.index, **fontdict_xtick)
