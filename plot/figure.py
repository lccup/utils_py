...
class DrawingBoard:
    ...
    def add_ax(self, x, y, width=1, height=1, rc=None, *args, **kwargs):
        ...
        """
        y = y + height
        x, y, w, h = self.point_and_length_to_absolute_coordinate(
            x, y, width, height)
        with plt.rc_context(rc=rc):
    ...
    def area_get_ax(self, index, order='C', with_ax=True, with_xy=False, *args, **kwargs):
        """"""
        ...
        if with_ax:
            ax = self.add_ax(x, y, self.area_width,
                             self.area_height, *args, **kwargs)
    ...
    def add_transparent_box(self,
        ...
        ax = self.add_ax(x, y, width, height,
                         rc=tl_rc(rc_box, rc_tl_notick, rc))
...
class A4Page(DrawingBoard):
    ...
    def __init__(
        ...
        assert nrows > 0 and nrows <= 46, "[Error] nrows need to be (0,46] , get {}".format(
            nrows)
