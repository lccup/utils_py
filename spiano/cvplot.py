...
def hex_to_bgr(hex_color):
    ...
    if len(hex_color) == 3:
        ...
        raise ValueError(
            "十六进制颜色代码必须是 3 或 6 位字符（如 '#FF0000' 或 'F00'）"
        )
...
def draw_boxes(img, boxes, color=COLORS["red"][2], thickness=2):
    def handel(box):
        cv2.rectangle(img, box[:2], box[2:], color=color, thickness=thickness)

    boxes = boxes.astype(int)
    np.apply_along_axis(lambda box: handel(box), axis=1, arr=boxes)
    return img


def draw_lines(img, points1, points2, color=COLORS["blue"][3], thickness=2):
...
def draw_points(img, points, radius=5, color=COLORS["blue"][3], thickness=-1):
...
def draw_boxes_relations(
    img,
    relations,
    boxes1,
    boxes2,
    color_box1=COLORS["red"][3],
    color_box2=COLORS["red"][3],
    ...
    cp2 = self.cal_center_point(boxes2).astype(int)
    ...
    for arr in relations:
        ...
        cv2.line(img, cp1[arr[0]], cp2[arr[1]], color=color_line, thickness=thickness)
    return img


def draw_points_relations(
    img,
    points1,
    points2,
    radius=5,
    ...
    np.apply_along_axis(
        lambda arr: handle(
            img, arr, radius, color_point1, color_point2, color_line, thickness
        ),
        axis=1,
        arr=points,
    )
...
def points2boxes(points1, points2):
    ...
    boxes[:, 3] = np.where(
        boxes[:, 3] - boxes[:, 1] < 20, boxes[:, 1] + 20, boxes[:, 3]
    )
